...
import io.f7z.olas.feature.profile.EditProfileScreen
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(
                ...
                val raw = back.arguments?.getString("profileJson").orEmpty()
                val profile = remember(raw) {
                    runCatching { Json.decodeFromString<OlasProfile>(raw) }.getOrNull()
                }
                EditProfileScreen(navController, currentProfile = profile)
            }
            composable(
                route = Routes.FULLSCREEN_IMAGE,
                arguments = listOf(navArgument("url") { type = NavType.StringType }),
            ) { back ->
                FullscreenImageScreen(navController, url = back.arguments?.getString("url").orEmpty())
            }

            // Settings
            composable(Routes.SETTINGS)          { SettingsScreen(navController) }
            composable(Routes.WOT_SETTINGS)      { WoTSettingsScreen(navController) }
            composable(Routes.RELAY_SETTINGS)    { RelaySettingsScreen() }
            composable(Routes.SERVER_SETTINGS)   { ServerSettingsScreen() }
        }
    }
}
