...
use nmp_nip68::{
    picture_acquisition_kinds, picture_feed_observer, picture_feed_predicate, ImageMeta,
    PictureEventRecord, PictureFeed, PictureFeedEntry,
};
...
pub(crate) fn photo_post_json_from_record(record: &PictureEventRecord) -> Option<String> {
    serde_json::to_string(&photo_post_from_record(record)).ok()
}
...
pub(crate) struct PhotoPost {
    id: String,
    author_pubkey: String,
    author_name: Option<String>,
    author_avatar: Option<String>,
    images: Vec<PhotoImageMeta>,
    caption: String,
    hashtags: Vec<String>,
    reaction_count: u32,
    comment_count: u32,
...
pub(crate) fn photo_post_from_record(record: &PictureEventRecord) -> PhotoPost {
    PhotoPost {
        id: record.event_id.clone(),
        author_pubkey: record.author.clone(),
        author_name: None,
        author_avatar: None,
        images: record.images.iter().map(photo_image_from_record).collect(),
...
fn photo_image_from_record(image: &ImageMeta) -> PhotoImageMeta {
    PhotoImageMeta {
        url: image.url.clone(),
        sha256: image.sha256.clone().unwrap_or_default(),
        mime: image
            .mime
            .clone()
            .unwrap_or_else(|| "image/jpeg".to_string()),
        width: image.dimensions.as_ref().map(|dim| dim.width),
        height: image.dimensions.as_ref().map(|dim| dim.height),
